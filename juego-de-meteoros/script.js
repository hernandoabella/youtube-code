window.onload = () => {
    ...
    function gameEnd(){
        ...
        document.getElementById("sco").innerHTML = "Puntuación : "+ score ;
    ...
    function scores(){
        ...
        score+=1;
            ...
            score+=0;
        } document.getElementById("p").innerHTML = "Puntuación : "+score;
    }
    ...
    function atk(){
        for(i=0;i<1;i++){
            enemies.push(new enemy(random()*cnv.width,0,"violet",3))
        }
    }

    function check(){
        for(let i = 0; i< enemies.length; i++){
            if(enemies[i].x > posX-15 && enemies[i].x < posX+15 && enemies[i].y >
                posY-15 && enemies[i].y < posY+15 ){    
                enemies = [];
                alive = false;
                gameEnd();
            }
